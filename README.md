...
portions of the playbook file may be used to install only certain pieces of it, such as Filebeat.
...
* Filebeat is used to collect log files from specific files, such as those generated by Apache, Nginx web server, and MySQL databases. It Logs info about the file system, including which files have changed and when.
* Metricbeat is used to collect system metrics from the operating system and services of the virtual machines running Metricbeat.
...
- Update the Ansible Inventory file `/etc/ansible/hosts` to include the following
```
[webservers]
